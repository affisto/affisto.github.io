...
function shownGlyph(hiragana) {
  return state.script === "hiragana" ? hiragana : toKatakana(hiragana);
}
...
function playable(glyph) {
  return Boolean(state.media[glyph]);
}
...
function columnLabels(sectionId) {
  if (sectionId === "contracted") return ["ゃ", "", "ゅ", "", "ょ"];
  if (sectionId === "sokuon") return ["っ", "", "", "", ""];
  return ["あ", "い", "う", "え", "お"];
}

function rowLabel(row, sectionId) {
  const voicedBases = { が: "か", ざ: "さ", だ: "た", ば: "は", ぱ: "は" };
  let label = voicedBases[row[0]] ?? row[0];
  if (sectionId === "contracted") label = [...label][0];
  return shownGlyph(label);
}
...
function selectGlyph(glyph, autoplay = false) {
  ...
  document.querySelector("#script-label").textContent =
    state.script === "hiragana" ? "히라가나" : "가타카나";
  ...
  document
    .querySelectorAll(".kana-key")
    .forEach((button) =>
      button.setAttribute("aria-pressed", String(button.dataset.glyph === glyph)),
    );
...
function renderSections() {
  ...
  container.replaceChildren(
    ...sections.map((section) => {
      ...
      article.className = "kana-section";
      ...
      heading.innerHTML += `<p>${available}개 영상</p>`;
      article.append(heading);
      const axis = document.createElement("div");
      axis.className = "kana-axis";
      axis.innerHTML = `<span>행 / 단</span><div>${columnLabels(section.id)
        .map((label) => `<span>${label ? `${shownGlyph(label)}단` : ""}</span>`)
        .join("")}</div>`;
      article.append(axis);
      section.rows.forEach((row) => {
        const wrap = document.createElement("div");
        wrap.className = "kana-row-wrap";
        const label = document.createElement("span");
        label.className = "kana-row-label";
        label.textContent = `${rowLabel(row, section.id)}행`;
        const rowElement = document.createElement("div");
        rowElement.className = `kana-row kana-row--${row.length}`;
        row.map(shownGlyph).forEach((glyph) => rowElement.append(createKey(glyph)));
        wrap.append(label, rowElement);
        article.append(wrap);
      });
      return article;
    }),
  ...
  document.querySelector("#kana-count").textContent =
    `${state.script === "hiragana" ? "히라가나" : "가타카나"} ${playableCount}개 영상`;
...
async function init() {
  ...
  document
    .querySelectorAll("[data-script]")
    .forEach((button) =>
      button.addEventListener("click", () => changeScript(button.dataset.script)),
    );
  document.querySelector("#prev-kana").addEventListener("click", () => {
    const items = playableGlyphs();
    const index = items.indexOf(state.selectedGlyph);
    selectGlyph(items[(index - 1 + items.length) % items.length], true);
  });
  document.querySelector("#next-kana").addEventListener("click", () => {
    const items = playableGlyphs();
    const index = items.indexOf(state.selectedGlyph);
    selectGlyph(items[(index + 1) % items.length], true);
  });
  document.querySelector("#replay-kana").addEventListener("click", () => {
    const video = document.querySelector("#kana-video");
    video.currentTime = 0;
    video.play().catch(() => {});
  });
...
init().catch((error) => {
  document.querySelector("#kana-sections").innerHTML = `<p class="kana-error">${error.message}</p>`;
});
